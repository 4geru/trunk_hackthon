def eventPostBack(event)
  ...
  when "allEvent"
    ...
    if Participant.where({user_id: user.id, event_id: events.id }).empty?
      m1.pushButton("参加する\u{1F44D}", {"data": "type=join&event_id=#{events.id}"}) 
    else
      m1.pushButton("キャンセルする\u{1F44E}", {"data": "type=leave&event_id=#{events.id}"})     
    end 
  ...
  when "confirm"
    ...
    if data["status"] == 'true'
      ...
      client.reply_message(event['replyToken'], [confirmSticky, m.reply("本当に「#{events.event_name}」を確定しますか？")])
    else
      client.reply_message(event['replyToken'], [sadSticky, { type: 'text', text: "「#{events.event_name}」をキャンセルしました。" }])
    end
  when "check"    
    events = Event.find(data["event_id"])
    user = User.where({user_id: event["source"]["userId"]}).first
    if data["status"] == 'true'
      ...
      client.reply_message(event['replyToken'], [happySticky, { type: 'text', text: "「#{events.event_name}」を確定しました。" }])
    else
      client.reply_message(event['replyToken'], [sadSticky, { type: 'text', text: "「#{events.event_name}」をキャンセルしました。" }])
    end
  end
